...
#define DRAW_TUNDERL              0x3000  // draw double underline text
    ...
    void (^drawTextFunc)(CGContextRef,CGRect,int) = ^(CGContextRef ctx, CGRect rowRect, int r){
        ...
        for (int c = 0; c < grid.cols; c++) {
            ...
            if (cell.textFlags & DRAW_UNDERC) {
                ...
                CGContextSetFillColor(ctx, COMPONENTS(cell.sp));
                CGContextFillRect(ctx, rect);
            } else if (cell.textFlags & DRAW_TUNDERL) {
                CGRect rect = CGRectMake(cellRect.origin.x, cellRect.origin.y+0.4*fontDescent, cellRect.size.width, 2);
                CGContextSetFillColor(ctx, COMPONENTS(cell.sp));
                CGContextFillRect(ctx, rect);
            }
